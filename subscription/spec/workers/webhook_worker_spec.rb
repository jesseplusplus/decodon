...
RSpec.describe Subscription::WebhookWorker do
  ...
  describe '#perform' do
    before do
      ...
      allow(contact).to receive(:account).and_return(user.account)
    ...
    it 'creates a subscription and invite for checkout.session.completed event' do
      ...
      stripe_sub = {
        ...
        billing_cycle_anchor: 1_688_153_740,
        ...
        created: 1_688_153_740,
        currency: 'usd',
        current_period_end: 1_690_745_740,
        current_period_start: 1_688_153_740,
        customer: 'cus_OB9PCVwIE2d1h2',
        days_until_due: nil,
    ...
    it 'creates a subscription and invites for members only for checkout.session.completed event with existing user' do
      event = {
        id: 'evt_1NTb67HGqYTGDgCuSglJHdf3',
        object: 'event',
        api_version: '2018-09-24',
      ...
      invite = Invite.first
      expect(sub.invite).to eq(invite)
      expect(invite.max_uses).to eq(2)
    end
    ...
    it 'sends a subscription canceled email for update events with cancel attributes changed' do
      event = {
        data: {
          object: {
            id: 'sub_1',
            cancel_at: 1_691_982_380,
            cancel_at_period_end: true,
            canceled_at: 1_689_779_903,
            cancellation_details: {
              comment: nil,
              feedback: nil,
