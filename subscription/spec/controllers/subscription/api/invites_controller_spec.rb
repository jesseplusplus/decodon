...
describe Subscription::Api::InvitesController do
  ...
  describe '#index' do
    it 'returns the invite associated with the users active subscription' do
      subscription = Fabricate(:stripe_subscription, user_id: user.id,
                                                     invite: Fabricate(:invite, max_uses: 1),
                                                     status: 'active')
    ...
    it 'returns invites for all active subscriptions' do
      subscription = Fabricate(:stripe_subscription, user_id: user.id,
                                                     invite: Fabricate(:invite, max_uses: 1),
                                                     status: 'active')
      subscription2 = Fabricate(:stripe_subscription, user_id: user.id,
                                                      invite: Fabricate(:invite, max_uses: 1),
                                                      status: 'active')
      subscription3 = Fabricate(:stripe_subscription, user_id: user.id,
                                                      invite: Fabricate(:invite, max_uses: 1),
                                                      status: 'canceled')
