...
const messages = defineMessages({
  ...
  showMemberList: {
    id: "status.show_member_list",
    defaultMessage: "Show member list",
  },
...
class ActionBar extends React.PureComponent {
  ...
  render() {
    ...
    const limitedByMe =
      status.get("visibility") === "limited" && status.get("circle_id");
    ...
    if (writtenByMe) {
      ...
      if (limitedByMe) {
        menu.push({
          text: intl.formatMessage(messages.showMemberList),
          action: this.handleMemberListClick,
        });
      }
