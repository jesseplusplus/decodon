...
export function updateTimeline(timeline, status, accept) {
  return (dispatch, getState) => {
    ...
    if (timeline === "home") {
      if (
        homeVisibilities.length == 0 ||
        homeVisibilities.includes(visibility)
      ) {
        insertTimeline("home");
...
export const expandHomeTimeline = ({ maxId, visibilities } = {}, done = noOp) =>
  expandTimeline(
    "home",
    "/api/v1/timelines/home",
    { max_id: maxId, visibilities: visibilities },
    done
  );
export const expandLimitedTimeline = (
  { maxId, visibilities } = {},
  done = noOp
) =>
  expandTimeline(
...
export const expandAccountTimeline = (accountId, { maxId, withReplies } = {}) =>
  expandTimeline(
    `account:${accountId}${withReplies ? ":with_replies" : ""}`,
    `/api/v1/accounts/${accountId}/statuses`,
    { exclude_replies: !withReplies, max_id: maxId }
  );
