...
export function normalizeStatus(status, normalOldStatus) {
  ...
  if (normalOldStatus) {
    ...
    normalStatus.visibility = normalStatus.limited
      ? "limited"
      : normalStatus.visibility;

    if (status.quote && status.quote.id) {
      const quote_spoilerText = status.quote.spoiler_text || "";
      const quote_searchContent = [quote_spoilerText, status.quote.content]
        .join("\n\n")
        .replace(/<br\s*\/?>/g, "\n")
        .replace(/<\/p><p>/g, "\n\n");
      ...
      const displayName =
        normalStatus.quote.account.display_name.length === 0
          ? normalStatus.quote.account.username
          : normalStatus.quote.account.display_name;
      normalStatus.quote.account.display_name_html = emojify(
        escapeTextContentForBrowser(displayName),
        quote_account_emojiMap
      );
      normalStatus.quote.search_index = domParser.parseFromString(
        quote_searchContent,
        "text/html"
      ).documentElement.textContent;
      let docElem = domParser.parseFromString(
        normalStatus.quote.content,
        "text/html"
      ).documentElement;
      ...
      normalStatus.quote.contentHtml =
        "<p>" +
        emojify(_contentHtml.substr(0, 150), quote_emojiMap) +
        (_contentHtml.substr(150) ? "..." : "") +
        "</p>";
      normalStatus.quote.spoilerHtml = emojify(
        escapeTextContentForBrowser(quote_spoilerText),
        quote_emojiMap
      );
      normalStatus.quote_hidden = expandSpoilers
        ? false
        : quote_spoilerText.length > 0 || normalStatus.quote.sensitive;
    }
  }
