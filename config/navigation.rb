...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |n|
    n.item :web, safe_join([material_symbol('chevron_left'), t('settings.back')]), main_app.root_path
    ...
    n.item :preferences, safe_join([material_symbol('settings'), t('settings.preferences')]), main_app.settings_preferences_path, if: -> { current_user.functional? && !self_destruct } do |s|
    ...
    n.item :relationships, safe_join([material_symbol('groups'), t('settings.relationships')]), main_app.relationships_path, if: -> { current_user.functional? && !self_destruct } do |s|
    ...
    n.item :filters, safe_join([material_symbol('filter_alt'), t('filters.index.title')]), main_app.filters_path, highlights_on: %r{/filters}, if: -> { current_user.functional? && !self_destruct }
    n.item :statuses_cleanup, safe_join([material_symbol('history'), t('settings.statuses_cleanup')]), main_app.statuses_cleanup_path, if: -> { current_user.functional_or_moved? && !self_destruct }

    n.item :security, safe_join([material_symbol('account_circle'), t('settings.account')]), main_app.edit_user_registration_path do |s|
    ...
    n.item :subscription, safe_join([material_symbol('diamond'), '   Subscription']), settings_subscription.subscriptions_path, highlights_on: %r{/settings/subscription}
    ...
    n.item :user_user_invites, safe_join([material_symbol('person_add'), t('invites.title')]), main_app.invites_path, if: -> { current_user.can?(:invite_users) && current_user.functional? && !self_destruct }
    n.item :development, safe_join([material_symbol('code'), t('settings.development')]), main_app.settings_applications_path, highlights_on: %r{/settings/applications}, if: -> { current_user.functional? && !self_destruct }

    n.item :trends, safe_join([material_symbol('trending_up'), t('admin.trends.title')]), main_app.admin_trends_statuses_path, if: -> { current_user.can?(:manage_taxonomies) && !self_destruct } do |s|
    ...
    n.item :moderation, safe_join([material_symbol('gavel'), t('moderation.title')]), nil, if: -> { current_user.can?(:manage_reports, :view_audit_log, :manage_users, :manage_invites, :manage_taxonomies, :manage_federation, :manage_blocks) && !self_destruct } do |s|
      s.item :reports, safe_join([material_symbol('flag'), t('admin.reports.title')]), main_app.admin_reports_path, highlights_on: %r{/admin/reports|admin/report_notes}, if: -> { current_user.can?(:manage_reports) }
    ...
    n.item :admin, safe_join([material_symbol('manufacturing'), t('admin.title')]), nil, if: -> { current_user.can?(:view_dashboard, :manage_settings, :manage_rules, :manage_announcements, :manage_custom_emojis, :manage_webhooks, :manage_federation) && !self_destruct } do |s|
      s.item :dashboard, safe_join([material_symbol('speed'), t('admin.dashboard.title')]), main_app.admin_dashboard_path, if: -> { current_user.can?(:view_dashboard) }
    ...
    n.item :sidekiq, safe_join([material_symbol('diamond'), 'Sidekiq']), main_app.sidekiq_path, link_html: { target: 'sidekiq' }, if: -> { current_user.can?(:view_devops) }
    n.item :pghero, safe_join([material_symbol('database'), 'PgHero']), main_app.pghero_path, link_html: { target: 'pghero' }, if: -> { current_user.can?(:view_devops) }
    n.item :logout, safe_join([material_symbol('logout'), t('auth.logout')]), main_app.destroy_user_session_path, link_html: { 'data-method' => 'delete' }
  end
end
